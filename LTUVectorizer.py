...
class LTUTransformer(TransformerMixin, BaseEstimator):
    ...
    def __init__(
        ...
        :param norm: {'l1', 'l2'}, default = 'l2'
            Each output row will have unit norm, either:
                - 'l2': Sum of squares of vector elements is 1. The cosine
            similarity between two vectors is their dot product when l2 norm has
            been applied
                - 'l1': Sum of absolute values of vector elements is 1
        :param use_idf: bool, default = True
            Enable inverse-document-frequency reweighting. If False, idf(t) = 1
        :param smooth_idf: bool, default = True
            Smooth idf weights by adding one to document frequencies,
            as if an extra document was seen containing every term in
            the collection exactly once. Prevents zero divisions.
        :param sublinear_tf: bool, default = False
            Apply sublinear tf scaling, i.e. replace tf with 1 + log(tf)
        '''
        #         if norm in ['l1', 'l2']:
        #             self.norm = norm
        #         else:
        #             raise NoSuchMethodError(f"molda doesn't sustain such norm as {norm}/nTry 'l1' or 'l2'")
    ...
    def transform(self, X, y=None, **fit_params):
        ...
        :return vectors : sparse matrix
            LTU-weighted document-term matrix.
        '''
        ...
        u = 1 / (0.8 + 0.2 * (X.toarray().shape[1] / np.mean(np.count_nonzero(X.toarray(), axis=1))))
        X = X * u
...
class LTUVectorizer(CountVectorizer):
    r"""Convert a collection of raw documents to a matrix of LTU features.
    Equivalent to :class:`CountVectorizer` followed by
    :class:`LTUTransformer`.
    ...
    # Broadcast the LTU parameters to the underlying transformer instance
    ...
    def idf_(self, value):
        ...
        if not hasattr(self, "_tfidf"):
            # We should support transferring `idf_` from another `LTUTransformer`
            # and therefore, we need to create the transformer instance it does not
            # exist yet.
            self._tfidf = LTUTransformer(
                #                 norm=self.norm,
    ...
    def transform(self, raw_documents):
        ...
        X : sparse matrix of (n_samples, n_features)
            Tf-idf-weighted document-term matrix.
        """
        check_is_fitted(self, msg="The LTU vectorizer is not fitted")
