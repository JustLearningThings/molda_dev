...
class PMIVectorizer(CountVectorizer): 
    ...
    def transform(self, raw_documents, y=None):
        check_is_fitted(self, attributes=['_obexp'], msg='The ObservedExpected instance is not fitted yet')
        self._bow = super().fit_transform(raw_documents)
        return np.log(self._obexp.transform(self._bow.toarray()))

    def fit(self, raw_documents, y=None):

      self._obexp  = ObservedExpectedTransformer()
      self._bow = super().fit_transform(raw_documents)
      self._obexp.fit(self._bow)
      return self
    
    def fit_transform(self, raw_documents, y=None):
      X = super().fit_transform(raw_documents)
      self._obexp = ObservedExpectedTransformer(epsilon=self.epsilon)
      return np.log(self._obexp.fit(X).transform(X.toarray())+self.epsilon)

 
class PPMIVectorizer(CountVectorizer): 
    def __init__(
        self,
        *,
        input="content",
    ...
    def transform(self, raw_documents, y=None):
        self._bow = super().fit_transform(raw_documents)
        check_is_fitted(self, attributes=['_obexp'], msg='The ObservedExpected instance is not fitted yet')
        X = np.log(self._obexp.transform(self._bow.toarray())+self.epsilon)
